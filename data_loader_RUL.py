...
class Load_Dataset(Dataset):
    ...
    def __init__(self, data, label, args):
        ...
        if isinstance(data, np.ndarray):
            self.x_data = tr.from_numpy(data).float()
            self.y_data = tr.from_numpy(label).float()
        else:
            self.x_data = data.float()
            self.y_data = label.float()


        self.len = data.shape[0]
        print(self.len)
        shape = self.x_data.size()
        self.x_data = self.x_data.reshape(shape[0],args.time_denpen_len, args.window_size, shape[2])
        self.x_data = tr.transpose(self.x_data, 3,2)
...
def data_generator(args, data_root, data_set, max_rul, seq_len, time_denpen_len, net_name=1):
    data_iter = CMPDataIter(data_root=data_root,
                            data_set=data_set,
                            max_rul=max_rul,
                            seq_len=seq_len * time_denpen_len,
                            net_name=net_name)
    out_x = data_iter.out_x
    out_ops = data_iter.out_ops
    out_y = data_iter.out_y

    cross_val_x = data_iter.cross_val_x
    cross_val_ops = data_iter.cross_val_ops
    cross_val_y = data_iter.cross_val_y

    test_x = data_iter.test_x
    test_ops = data_iter.test_ops
    test_y = data_iter.test_y

    train_dataset = Load_Dataset(out_x, out_y, args)
    valid_dataset = Load_Dataset(cross_val_x, cross_val_y, args)
    test_dataset = Load_Dataset(test_x, test_y, args)

    train_loader = tr.utils.data.DataLoader(dataset=train_dataset, batch_size=args.batch_size,
                                               shuffle=True, drop_last=args.drop_last,
                                               num_workers=0)
    valid_loader = tr.utils.data.DataLoader(dataset=valid_dataset, batch_size=args.batch_size,
                                               shuffle=False, drop_last=args.drop_last,
                                               num_workers=0)

    test_loader = tr.utils.data.DataLoader(dataset=test_dataset, batch_size=args.batch_size,
...
if __name__ =='__main__':
    ...
    # train, valid, test = data_generator(args,'D://PhD_in_Sg/research related/datasets/RUL',
    #                         data_set='FD001',
    #                         max_rul=125,
    ...
    data_iter = CMPDataIter(data_root='D://PhD_in_Sg/research related/datasets/RUL',
                            data_set='FD001',
                            max_rul=130,
                            seq_len=160,
                            net_name=1)

    data = data_iter.train_x[0]
    color = ['black', 'red', 'chocolate', 'olivedrab', 'blue', 'orchid','slategray']
    for i in range(6):
        plt.plot(data[:,i],c=color[i], linewidth = 4)
        plt.savefig('./signals/{}.png'.format(i), dpi=600, format='png')
        plt.show()
