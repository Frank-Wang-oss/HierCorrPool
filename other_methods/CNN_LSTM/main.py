...
class Train():
    def __init__(self, args):
        ...
        self.net = Model.CNN_LSTM(args.num_nodes, args.conv_length,args.window_size*args.time_denpen_len, args.num_class)
        # self.net = Model_Graph.Node_Classification(input_dimension=args.window_size,
    ...
    def Trans_dimension(self, X):
        ## input size is (bs, time_length, num_sensors, input_dimension)
        bs, time_length, num_sensors, input_dimension = X.size()
        X = tr.transpose(X, 2, 3)
        X = tr.reshape(X, [bs, -1, num_sensors])
        return X
    ...
    def Train_batch(self):
        ...
        for data, label in self.train:
            data = self.Trans_dimension(data)
            # print(data.size())
    ...
    def Train_model(self):
        ...
        for i in range(epoch):
            ...
            print('Training one epoch is {}'.format(time.time() - time0))
    ...
    def Prediction(self):
        ...
        for data, label in self.test:
            ...
            predicted_.append(predicted.detach())
            real_.append(label)
        print('Inference time for one sample is {}'.format((time.time()-time0) / len(self.test)))
...
if __name__ == '__main__':
    ...
    def args_config_ISRUC(args):
        ...
        args.embedding_dimension = 10
        args.num_class = 5
        args.conv_length = 5317

        return args


    # window_size_choices = [5,6,7,8,9,10]
    # time_denpen_len_choices = [5,6,7,8,9,10]
    # datasets = ['RUL', 'HAR', 'ISRUC']
    # for dataset in datasets:



    for datasub in [1,2,3,4]:
        args = args_config_RUL(datasub, args)
        for i in range(30):
            args.save_name = 'CNN_LSTM_RUL_datasub{}_{}'.format(datasub, i)
            train = Train(args)
            train.Train_model()
    ...
    for i in range(30):
        args.save_name = 'CNN_LSTM_HAR_{}'.format(i)
        train = Train(args)
        train.Train_model()
        print(args.save_name)
    ...
    for num_subject in range(10):
        ...
        for i in range(3):
            args.save_name = 'CNN_LSTM_{}_sub{}_{}'.format('ISRUC',num_subject, i)
            train = Train(args)
            train.Train_model()
            print(args.save_name)
