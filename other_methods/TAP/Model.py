...
def Local_Voting(adj, X):
    sim_R = torch.bmm(X, torch.transpose(X, -1, -2))

    R_p = sim_R * adj

    rank_score = torch.mean(R_p, -1).squeeze()

    rank_score = torch.softmax(rank_score, -1)
...
class Graph_Pooling(nn.Module):
    def __init__(self, input_dimension, out_dimension, out_nodes):
        ...
        self.global_voting = nn.Sequential(
    ...
    def forward(self, adj, X):
        X_out = self.fc(torch.bmm(adj, X))

        adj_X = torch.bmm(adj, X)
        adj_X = self.global_voting(adj_X).squeeze()
        global_voting = torch.softmax(adj_X, -1)

        local_voting = Local_Voting(adj, X)

        Voting = global_voting + local_voting

        _, idx = torch.sort(Voting, descending=True, dim=1)

        topk = idx[:, :self.out_nodes]
...
class TAP(nn.Module):
    def __init__(self, input_dimension, hidden_dimension, embedding_dimension, time_length, args):
        ...
        if args.layers == 1:
            self.gc1 = Graph_Pooling(embedding_dimension * time_length, embedding_dimension * time_length * 3,
                                       args.num_sensor_last)
        elif args.layers == 2:
            self.gc1 = Graph_Pooling(embedding_dimension * time_length, embedding_dimension * time_length* 2,
                                                             args.hidden_node1)
            self.gc2 = Graph_Pooling(embedding_dimension * time_length * 2, embedding_dimension * time_length * 3,
                                                             args.num_sensor_last)
        elif args.layers == 3:
            self.gc1 = Graph_Pooling(embedding_dimension * time_length, embedding_dimension * time_length* 2,
                                                             args.hidden_node1)
            self.gc2 = Graph_Pooling(embedding_dimension * time_length * 2, embedding_dimension * time_length * 3,
                                                             args.hidden_node2)
            self.gc3 = Graph_Pooling(embedding_dimension * time_length * 3, embedding_dimension * time_length * 3,
                                                             args.num_sensor_last)
